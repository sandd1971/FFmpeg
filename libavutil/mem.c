...
#undef av_reallocp
#undef av_realloc_f
#undef av_realloc_array
#undef av_reallocp_array
#undef av_fast_realloc
...
#undef av_strndup
#undef av_memdup
#undef av_memcpy_backptr
#undef av_dynarray_add
#undef av_dynarray_add_nofree
#undef av_dynarray2_add

void *av_malloc(size_t size, const char* f, int l)
{
    void *ptr = NULL;
    ...
    if (posix_memalign(&ptr, ALIGN, size))
        ptr = NULL;
#elif HAVE_ALIGNED_MALLOC
    ptr = _aligned_malloc_dbg(size, ALIGN, f, l);
#elif HAVE_MEMALIGN
#ifndef __DJGPP__
    ptr = memalign(ALIGN, size);
#else
...
void *av_realloc(void *ptr, size_t size, const char* f, int l)
{
    void *ret;
    if (size > atomic_load_explicit(&max_alloc_size, memory_order_relaxed))
        return NULL;
...
#if HAVE_ALIGNED_MALLOC
    ret = _aligned_realloc_dbg(ptr, size + !size, ALIGN, f, l);
#else
    ret = _realloc_dbg(ptr, size + !size, f, l);
#endif
#if CONFIG_MEMORY_POISONING
    if (ret && !ptr)
        memset(ret, FF_MEMORY_POISON, size);
...
void *av_realloc_f(void *ptr, size_t nelem, size_t elsize, const char* f, int l)
{
    ...
    r = av_realloc(ptr, size, f, l);
    if (!r)
        av_free(ptr);
    return r;
}

int av_reallocp(void *ptr, size_t size, const char* f, int l)
{
    void *val;
...
void *av_malloc_array(size_t nmemb, size_t size, const char* f, int l)
{
    size_t result;
    if (size_mult(nmemb, size, &result) < 0)
        return NULL;
    return av_malloc(result, f, l);
}

#if FF_API_AV_MALLOCZ_ARRAY
void *av_mallocz_array(size_t nmemb, size_t size, const char* f, int l)
{
    size_t result;
    if (size_mult(nmemb, size, &result) < 0)
        return NULL;
    return av_mallocz(result, f, l);
}
#endif

void *av_realloc_array(void *ptr, size_t nmemb, size_t size, const char* f, int l)
{
    size_t result;
    if (size_mult(nmemb, size, &result) < 0)
        return NULL;
    return av_realloc(ptr, result, f, l);
}
...
int av_reallocp_array(void *ptr, size_t nmemb, size_t size, const char* f, int l)
{
    ...
    memcpy(&val, ptr, sizeof(val));
    val = av_realloc_f(val, nmemb, size, f, l);
    memcpy(ptr, &val, sizeof(val));
    if (!val && nmemb && size)
        return AVERROR(ENOMEM);
...
void *av_mallocz(size_t size, const char* f, int l)
{
    void *ptr = av_malloc(size, f, l);
    if (ptr)
        memset(ptr, 0, size);
    return ptr;
}

void *av_calloc(size_t nmemb, size_t size, const char* f, int l)
{
    size_t result;
    if (size_mult(nmemb, size, &result) < 0)
        return NULL;
...
char *av_strdup(const char *s, const char* f, int l)
{
    char *ptr = NULL;
    if (s) {
        size_t len = strlen(s) + 1;
        ptr = av_realloc(NULL, len, f, l);
        if (ptr)
            memcpy(ptr, s, len);
    }
    return ptr;
...
char *av_strndup(const char *s, size_t len, const char* f, int l)
{
    char *ret = NULL, *end;
...
int av_dynarray_add_nofree(void *tab_ptr, int *nb_ptr, void *elem, const char* f, int l)
{
    void **tab;
    memcpy(&tab, tab_ptr, sizeof(tab));
...
void *av_dynarray2_add(void **tab_ptr, int *nb_ptr, size_t elem_size,
                       const uint8_t *elem_data, const char* f, int l)
{
    uint8_t *tab_elem_data = NULL;
...
void *av_fast_realloc(void *ptr, unsigned int *size, size_t min_size, const char* f, int l)
{
    ...
    ptr = av_realloc(ptr, min_size, f, l);
    /* we could set this to the unmodified min_size but this is safer
     * if the user lost the ptr and uses NULL now
     */
    if (!ptr)
...
static inline void fast_malloc(void *ptr, unsigned int *size, size_t min_size, int zero_realloc, const char* f, int l)
{
    size_t max_size;
    void *val;
