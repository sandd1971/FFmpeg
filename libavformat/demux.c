                        ...
                        int stream_index, int flush)
{
    ...
    while (size > 0 || (flush && got_output)) {
        ...
        out_pkt->pict_type    = sti->parser->pict_type;
        out_pkt->picture_structure = sti->parser->picture_structure;
        out_pkt->output_picture_number = sti->parser->output_picture_number;
        out_pkt->nb_fields =
            (sti->parser->repeat_pict == 1 &&
            (sti->parser->picture_structure == AV_PICTURE_STRUCTURE_TOP_FIELD ||
             sti->parser->picture_structure == AV_PICTURE_STRUCTURE_BOTTOM_FIELD)) ?
            1 : sti->parser->repeat_pict + 1;
...
return_packet:
    ...
    FFStream *const sti = ffstream(st);
    if (ret >= 0 && st->codecpar && sti && sti->avctx)
